...
void command_transmit_message(int& connection, MYSQL& mysql) {
    ...
    ssize_t bytes = write(connection, (char*) &message_accepted, size_of_bool);
    if (bytes == 0) {
        cout << CONNECTION_LOST_SERVER_MSG << endl;
        // TODO: HANDLE LOST CONNECTION
    }
...
void command_get_messages(int &connection, char login[], MYSQL &mysql) {
    std::vector<Message> show_messages =
        read_messages_from_db(n_msgs_on_screen, login, mysql);
    int bytes{0};
    for (auto& message : show_messages) {
        bytes = write(connection, message.getMessage().c_str(), msg_size);
        if (bytes <= 0) {
            // cout << "Connection error\n";
            break;
        }
        bytes = write(connection, message.getAuthor().c_str(), usr_size);
        if (bytes <= 0) {
            // cout << "Connection error\n";
            break;
        }
        bytes = write(connection, message.forWhom().c_str(), usr_size);
        if (bytes <= 0) {
            // cout << "Connection error\n";
            break;
        }
    ...
    char end[] = "end";
    bytes = write(connection, end, msg_size);
    if (bytes <= 0) {
        cout << "Connection error\n";
    }
}
